...
while len(pairs_found) < rows * columns // 2:
    funciones.print_board(board, rows, columns, pairs_found)
    ...
    try:
        ...
        if card1 == card2:
            ...
            funciones.pairs_found.add((row1, col1))
            funciones.pairs_found.add((row2, col2))
        else:
...
if __name__ == "__main__":
    rows = 4
    columns = 4
    funciones.memory_game(rows, columns)
...
print({user_data.get('name')}, ' tiene ', {user_data.get('age')}, ' años, vive en ', {user_data.get('address')}, ' y su número de teléfono es ', {user_data.get('phone_number')})
